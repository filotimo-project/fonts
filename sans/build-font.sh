...
download_source() {
  rm --force "${ARCHIVE}"
  curl -fsSLO "${BASE_URL}/v${INTER_VERSION}/${ARCHIVE}"
  echo "${HASH} ${ARCHIVE}" | \
...
build_font() {
  pyftfeatfreeze \
    --features "cv05" \
    --replacenames "Inter Variable/Filotimo Sans" \
    "${1}" "${2}"
  rm "${1}"
}
...
download_source
extract_fonts
build_font InterVariable.ttf FilotimoSans-Regular.ttf
build_font InterVariable-Italic.ttf FilotimoSans-Italic.ttf
